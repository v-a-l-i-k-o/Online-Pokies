...
var path = {
    built: {
     css_bootstrap: '../built/css/bootstrap/',
    ...
    src: {
     css_bootstrap: '../src/styles/bootstrap/bootstrap.less',
      js_bootstrap: '../src/js/bootstrap.js',
        js_plugins: '../src/js/plugins.js',
           ...
           js_main: '../src/js/main.js',
             fonts: '../src/fonts/**/*.*',
              html: '../src/*.html',
               css: '../src/styles/custom/main.less',
               img: '../src/img/*.*'
    },

    watch: {
        styles: '../src/styles/custom/**/*.less',
         fonts: '../src/fonts/**/*.*',
          html: '../src/**/*.html',
           img: '../src/img/**/*.*',
            js: '../src/js/**/*.js'
...
gulp.task('js_bootstrap:build', function () {
...
gulp.task('styles:build', function () {
    gulp.src(path.src.css)                   // Выберем файл main.less
        .pipe(plumber())                     // Ловим ошибки для предотвращения остановки gulpa
...
gulp.task('styles_bootstrap:build', function () {
    gulp.src(path.src.css_bootstrap)               // Выберем файл bootstrap.less
        .pipe(plumber())                           // Ловим ошибки для предотвращения остановки gulpa
        .pipe(less())                              // Компилируем через препроцессор
        .pipe(nano())                              // Сожмём файл
        .pipe(rename('bootstrap.min.css'))         // Переименуем
        .pipe(gulp.dest(path.built.css_bootstrap)) // Выплюнем в build
});
...
gulp.task('fonts:build', function () {
    gulp.src(path.src.fonts)                 // Выберем шрифты
        .pipe(changed(path.built.fonts))     // Получаем файлы и пропускаем только изменившиеся
...
gulp.task('build', ['html:build', /*'styles_bootstrap:build',*/ 'styles:build', /*'js_bootstrap:build',*/ 'js:build', 'img:build', 'fonts:build']);
